...
function Picked_Network() {
		...
		# User prompt to enter the target network range for scanning.
...
# Requesting Domain name and Active Directory (AD) credentials.
...
# User prompt to choose a password list, defaulting to Rockyou if none is specified.
function Pass_list() {
...
# Identifying the IP Address of the DHCP server.
...
function Key_Services() {
	...
	echo
			...
			# Enumerating for shared folders.
...
# requires AD Credentials
	...
	echo
			...
			# 3 NSE scripts relevant for enumerating domain networks.
function NSE_SCRIPTS() {
	...
	# Advanced (Only if AD credentials were entered):	
...
# To select a desired operation level (Basic, Intermediate, Advanced or None) for each mode: Scanning, Enumeration, Exploitation. Note: Selection of a higher level automatically encompasses the capabilities of the preceding levels. 
function Levels() {
